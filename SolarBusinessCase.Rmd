...
forecast$Bill.Total <- as.numeric(gsub("[$]", "", forecast$Bill.Total))
...
### Usage Data (Apr 2029 to July 2022)
...
### Calculate 25 yr Net Present Value

- Blue Raven: `r sum(npv_data$offset_blue[1:26]*(1-inflation)^(0:25))`
- EcoHouse: `r sum(npv_data$offset_eco[1:26]*(1-inflation)^(0:25))`
...
#### Actual Temp since Installation
...
ggplot(X2 |>
         ...
         dplyr::mutate(variable = factor(variable, labels = c("Forecast Total Demand (kWh)",
                                                              "From Grid (kWh)",
                                                              "To Grid (kWh)",
...
```{r}
temp <- X2 |>
  dplyr::mutate(total_cash_flow = cumsum(total_cash_flow)) |>
  dplyr::filter(Month == 12)
ggplot(temp, aes(x = Year, y = total_cash_flow)) + 
  geom_point() +
  geom_line() +
...
- 20 yr NPV: `r ind <- which(X2$Year == 2042 & X2$Month == 8); scales::dollar(X2$npv[ind])`
- 25 yr NPV: `r ind <- which(X2$Year == 2047 & X2$Month == 8); scales::dollar(X2$npv[ind])`
